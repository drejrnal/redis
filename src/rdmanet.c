...
static void rdmaDestroyIOBuffer(rdmaContext *ctx) {
    if (ctx->send_region) {
        if (ibv_dereg_mr(ctx->send_region)) {
            serverLog(LL_WARNING, "server de-register send memory region failed:%s", strerror(errno));
        }
        ctx->send_region = NULL;
    }
    if (ctx->recv_region) {
        if (ibv_dereg_mr(ctx->recv_region)) {
            serverLog(LL_WARNING, "server de-register receive memory region failed:%s", strerror(errno));
        }
        ctx->recv_region = NULL;
    }
    if (ctx->cmd_region) {
        if (ibv_dereg_mr(ctx->cmd_region)) {
            serverLog(LL_WARNING, "server de-register cmd memory region failed:%s", strerror(errno));
        }
        ctx->cmd_region = NULL;
    }
...
static int rdmaSetupIOBuffer(rdmaContext *ctx, struct rdma_cm_id *cmid) {
    ...
    for (int i = 0; i < REDIS_MAX_SGE; i++) {
        ...
        if (rdmaPostRecv(ctx, cmid, cmd)) {
            serverLog(LL_WARNING, "server post receive failed");
            goto destroy_buffer;
        }
    }
...
static int rdmaAdjustSendBuffer(rdmaContext *ctx, uint32_t length) {
    int ret = C_OK, access_flags = 0;
    uint32_t cur_length = ctx->send_length;
    ...
    ctx->send_region = ibv_reg_mr(ctx->pd, ctx->send_buffer,
                                  length, access_flags);
    if (!ctx->send_region) {
        serverLog(LL_WARNING, "server register send memory region failed");
        zfree(ctx->send_buffer);
        ctx->send_length = 0;
        ctx->send_buffer = NULL;
        ret = C_ERR;
...
static int ibVerbsWrite(connection *c, size_t write_len) {
    ...
    //selective signaled
    wr.send_flags = ((++ctx->send_ops) % (REDIS_MAX_SGE / 2)) ? 0 : IBV_SEND_SIGNALED;
    ...
    serverLog(LL_DEBUG, "Redis server write %ld bytes of data", write_len);
...
static int connRdmaWrite(connection *c, const void *data, size_t data_len) {
    ...
    if (c->state == CONN_STATE_CLOSED || c->state == CONN_STATE_ERROR)
        return 0;
...
static int connRdmaRead(connection *c, void *buf, size_t buf_len) {
    ...
    serverLog(LL_DEBUG, "Redis server read %ld bytes of data", nread);
...
static int rdmaHandleRecv(rdmaContext *ctx, struct rdma_cm_id *cmid, rdmaCommand *cmd) {
    ...
    if (rdmaPostRecv(ctx, cmid, cmd)) {
        serverLog(LL_WARNING, "Redis server post recv failed after handle recv wr:%s", strerror(errno));
        ret = C_ERR;
    }
    return ret;
}
...
static int rdmaHandleCq(rdmaConnection *rdma_conn) {
    ...
    ret = ibv_get_cq_event(comp_channel, &cq, &black_hole);
    //above operation is non-block, so it's necessary to check EAGAIN errno.
    if (ret) {
        if (errno != EAGAIN) {
            ret = C_ERR;
            rdma_conn->conn.last_errno = errno;
            serverLog(LL_WARNING, "get event notification from comp channel failed:%s",
                      connGetLastError(&rdma_conn->conn));
            return C_ERR;
        }
        return C_OK;
    }
    /*
     * there exists such circumstances such that get_cq_event() returns successfully while poll cq returns 0
     * so it's necessary to ack cq events after get_cq_event() successfullly returns rather than poll_cq() returns,
     * otherwise, even one missing ack could cause ibv_destroy_cq() run forever and never returns(whether error happend or not).
     *
     */
    //ibv_ack_cq_events(cq, 1);
    ctx->ack_ops +=1;
    if( ctx->ack_ops == ACK_MAX_NUM ) {
        ibv_ack_cq_events(cq, ctx->ack_ops);
        ctx->ack_ops = 0;
    }
    ...
    do {
        cq_events = ibv_poll_cq(cq, 1, &wc);
        if (cq_events == 0) {
            continue;
        }
        if (wc.status != IBV_WC_SUCCESS) {
            serverLog(LL_WARNING, "Redis server handle cq event failed:%s, %d", strerror(errno), wc.status);
            return C_ERR;
        }
        switch (wc.opcode) {
            case IBV_WC_RECV_RDMA_WITH_IMM:
                cmd = (rdmaCommand *) (wc.wr_id);
                if (rdmaHandleRecvIMM(ctx, cmid, cmd, wc.byte_len)) {
                    rdma_conn->conn.state = CONN_STATE_ERROR;
                    return C_ERR;
                }
                serverLog(LL_DEBUG, "Redis server receive %d amount of data from client", wc.byte_len);
                break;

            case IBV_WC_RECV:
                cmd = (rdmaCommand *) (wc.wr_id);
                serverLog(LL_DEBUG, "Redis server receive cmd meta info:[dest addr:%lx,length:%d, RKEY:%d]",
                          ntohu64(cmd->addr), ntohl(cmd->length),
                          ntohl(cmd->rkey));
            ...
            case IBV_WC_SEND:
                cmd = (rdmaCommand *) (wc.wr_id);
                serverLog(LL_DEBUG, "Redis server send cmd length:%d, RKEY:%d", ntohl(cmd->length), ntohl(cmd->rkey));
                rdmaHandleSend(cmd);
                serverLog(LL_DEBUG, "Redis server send command with recv buffer addr %lx, length %d",
                          ntohu64(cmd->addr), ntohl(cmd->length));
                break;
            case IBV_WC_RDMA_WRITE:
                rdmaHandleWrite(wc.byte_len);
                serverLog(LL_DEBUG, "Redis server write %d amount of data to client", wc.byte_len);
                break;
            default:
                serverLog(LL_WARNING, "Redis server unexpected opcode %d", wc.opcode);
                return C_ERR;
        }
    } while (cq_events);
    /*for (int i = 0; i < cq_events; i++) {
        if (wc[i].status != IBV_WC_SUCCESS) {
            serverLog(LL_WARNING, "Redis server handle cq event failed:%s, %d", strerror(errno), wc[i].status);
            //continue;
            return C_ERR;
        }
        switch (wc[i].opcode) {
            case IBV_WC_RECV_RDMA_WITH_IMM:
                ...
                serverLog(LL_DEBUG, "Redis server receive %d amount of data from client", wc[i].byte_len);
                break;

            case IBV_WC_RECV:
                cmd = (rdmaCommand *) (wc[i].wr_id);
                serverLog(LL_DEBUG, "Redis server receive cmd length:%d, RKEY:%d", ntohl(cmd->length),
                          ntohl(cmd->rkey));
                if (rdmaHandleRecv(ctx, cmid, cmd)) {
                    rdma_conn->conn.state = CONN_STATE_ERROR;
                    return C_ERR;
                }
                serverLog(LL_DEBUG, "Redis server receive command with from client with recv buffer:%lx",
                          ntohu64(cmd->addr));
                break;
            ...
            case IBV_WC_RDMA_WRITE:
                ...
                serverLog(LL_DEBUG, "Redis server write %d amount of data to client", wc[i].byte_len);
                break;
            default:
                serverLog(LL_WARNING, "Redis server unexpected opcode %d", wc[i].opcode);
                return C_ERR;
        }
    }*/
...
int connRdmaCron(struct aeEventLoop *eventLoop, long long id, void *clientData) {
    ...
    //rdmaHandleCq(c);
    serverLog(LL_DEBUG, "Redis server time event occured:");
...
static int rdmaHandleConnectRequest(struct rdma_cm_event *event, char *ip, size_t ip_len, int *port) {
    ...
    rdmaContext *ctx = zcalloc(sizeof(rdmaContext));
    ctx->timeevent = -1;
...
static int rdmaHandleEstablished(struct rdma_cm_event *event) {
    ...
    serverLog(LL_DEBUG, "Redis server handle RDMA_CM_EVENT_ESTABLISHMENT event completion");
    return C_OK;
}
//handle DISCONNECTED
static int rdmaHandleDisconnection(struct rdma_cm_event *event) {
    ...
    serverLog(LL_DEBUG, "Redis server receive disconnection from peer node");
    ...
    if (conn->read_handler != NULL)
        callHandler(conn, conn->read_handler);
    else if (conn->write_handler != NULL)
        callHandler(conn, conn->write_handler);
...
static void connRdmaClose(connection *conn) {
    ...
    if( ctx->ack_ops < ACK_MAX_NUM)
        ibv_ack_cq_events(ctx->cq, ctx->ack_ops);
    ...
    if (rdma_destroy_id(cmid))
        serverLog(LL_WARNING, "Redis server[connRdmaClose] destroy connection id failed:%s", strerror(errno));
...
int netRdmaAccept(char *err, int s, char *ip, size_t ip_len, int *port, void **privdata) {
    ...
    if (rdma_get_cm_event(listen_channel, &event)) {
        if (errno != EAGAIN) {
            serverLog(LL_WARNING, "Redis server listen for client fd %d failed:%s", s, strerror(errno));
            return ANET_ERR;
        }
        return ANET_OK;
    }
    ...
    switch (event->event) {
        case RDMA_CM_EVENT_CONNECT_REQUEST:
            ...
            if (ret == C_OK) {
                *privdata = event->id;
                ...
                serverLog(LL_WARNING, "Redis server handle RDMA_CM_EVENT_CONNECT_REQUEST failed");
                ret = ANET_ERR;
            }
            break;
        ...
        case RDMA_CM_EVENT_TIMEWAIT_EXIT:
            serverLog(LL_NOTICE, "Redis server exit time wait status");
            break;
        ...
        default:
            ret = ANET_OK;
            serverLog(LL_NOTICE, "Redis server listening channel ignore events:%s", rdma_event_str(event->event));
            break;
    }
